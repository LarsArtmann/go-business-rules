...
5. **Pin churn I caused myself:** re-pinned stats to 30 _before_ applying my own `//nolint` suppression, which immediately moved the count to 29 — one wasted cycle that measuring after the suppression would have avoided.
...
_(Brainstorm per skill guidance — most items below the first ~15 are ROADMAP fuel, not commitments.)_
