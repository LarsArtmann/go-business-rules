...
| Repo **private** ⟪2026-09-18: SUPERSEDED — repo went PUBLIC 2026-09-17; pkg.go.dev now indexes v2.2.0⟫; proxy/pkg.go.dev serve nothing                                            | `gh repo view --json visibility`; proxy `@v/list` 404                                                                                                      |
...
Polish-Customs integration (the only true external validation of the breaking-change surface), Datastar leg, OTel listener — the parts that make events/streaming/context _real_.
...
| 2026-09-14 | EXECUTED (concurrent session executed tasks 1-14 its own way: `/v2` path + local `v2.1.0` tag, re-pins, OTel, Datastar, check-all, CI billing diagnosis). Execution session completed the rest: v2.1.0 pushed + real-remote publish verification (`2.1.0 true`); Polish-Customs `replace` dropped, suite green against published v2.1.0; CI re-enabled (billing remains the documented blocker); tasks 15-19/22-23 shipped (string/time/network/precision builders, Not/Or/Xor, rule metadata on RuleEvaluated, gopter Build≡Stream property test); 20-21 shipped (hygiene files, ADR-0002/3/4); 24 upstreamed (level-aware annotate-rows scoping + occurrence suffix); 25 done (ROADMAP pruned, art-dupl re-verified at 0 clones). Pins re-measured: 25 PHANTOM / 29 stats. Suite 251/251 green (property test opt-in), coverage 97.1%, check-all ALL MODULES GREEN, format gate green. |
