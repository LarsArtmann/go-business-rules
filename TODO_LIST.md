...
_The 2026-09-14 v2.1.0 session resolved: the unresolvable `v2.0.0` tag (module path migrated to `/v2`), the stale branching-flow pins (re-pinned to analyzer reality: 18 PHANTOM, 22 stats as of that session; 25 PHANTOM, 29 stats after the same-day builder batch — pins track current-tree reality by policy), `WithConcurrency(n)`, context-carrying rules (`ContextRule` + `NewContextRule`), the `listeners/otel` module, the Datastar rewrite of `examples/sse`, Stream benchmarks, goroutine-leak regression tests, the all-modules `check-all` flake app, the CI module matrix, and Polish-Customs compatibility verification against `/v2`. All recorded in [`CHANGELOG.md`](CHANGELOG.md)._
