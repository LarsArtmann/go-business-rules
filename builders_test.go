...
var _ = Describe("Builders", func() {
	Describe("Numeric Builders", func() {
		...
		It("should validate MinInt", func() {
			Expect(
				businessrules.MinInt("val", 5, 5, businessrules.SeverityError).Check(),
			).To(Succeed())
			Expect(
				businessrules.MinInt("val", 10, 5, businessrules.SeverityError).Check(),
			).To(Succeed())
			Expect(
				businessrules.MinInt("val", 3, 5, businessrules.SeverityError).Check(),
			).ToNot(Succeed())
		})
